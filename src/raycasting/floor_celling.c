...
void	init_struct_floor(t_data *data, int y)
{
	...
	data->floor_c->floor_step_x = data->floor_c->row_dist * (data->floor_c->ray_dir_x1 - data->floor_c->ray_dir_x0) / M_W;
	data->floor_c->floor_step_y = data->floor_c->row_dist * (data->floor_c->ray_dir_y1 - data->floor_c->ray_dir_y0) / M_W;	
	data->floor_c->floor_x = data->player.position[0] + data->floor_c->row_dist * data->floor_c->ray_dir_x0;
	data->floor_c->floor_y = data->player.position[1] + data->floor_c->row_dist * data->floor_c->ray_dir_y0;
}
...
void	draw_pixel_floor_celling(t_data *data, int x, int y)
{
	...
	if (data->floor_c->is_floor)
	{
		color = ft_get_pixel_img(data->line->texture[4], tex_floor.x, tex_floor.y);
		my_mlx_pixel_put(data->img, x, y, color);
	}
	else
	{
		color = ft_get_pixel_img(data->line->texture[5], tex_floor.x, tex_floor.y);
		my_mlx_pixel_put(data->img, x, y, color);
	}
}
